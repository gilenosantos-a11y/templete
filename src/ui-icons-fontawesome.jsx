<>
  {"{"}% set title = 'Fontawesome' %{"}"}
  {"{"}% set filename = 'ui-icons-fontawesome.html' %{"}"}
  {"{"}% extends 'src/layouts/master.html' %{"}"}
  {"{"}% block content %{"}"}
  <div className="page-heading">
    <div className="page-title">
      <div className="row">
        <div className="col-12 col-md-6 order-md-1 order-last">
          <h3>Fontawesome</h3>
          <p className="text-subtitle text-muted">
            Get over 1,000+ icons with Fontawesome.
          </p>
        </div>
        <div className="col-12 col-md-6 order-md-2 order-first">
          <nav
            aria-label="breadcrumb"
            className="breadcrumb-header float-start float-lg-end"
          >
            <ol className="breadcrumb">
              <li className="breadcrumb-item">
                <a href="index.html">Dashboard</a>
              </li>
              <li className="breadcrumb-item active" aria-current="page">
                Fontawesome
              </li>
            </ol>
          </nav>
        </div>
      </div>
    </div>
    <section className="section">
      <div className="card">
        <div className="card-header">
          <h4 className="card-title">Icon List</h4>
        </div>
        <div className="card-body">
          <div className="row fontawesome-icons">
            <article
              id="ad"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ad
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f641
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="address-book"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  address-book
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2b9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="address-card"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  address-card
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2bb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="adjust"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  adjust
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f042
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="air-freshener"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  air-freshener
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5d0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="align-center"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  align-center
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f037
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="align-justify"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  align-justify
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f039
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="align-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  align-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f036
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="align-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  align-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f038
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="allergies"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  allergies
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f461
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ambulance"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ambulance
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0f9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="american-sign-language-interpreting"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  american-sign-language-interpreting
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2a3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="anchor"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  anchor
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f13d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="angle-double-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  angle-double-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f103
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="angle-double-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  angle-double-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f100
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="angle-double-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  angle-double-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f101
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="angle-double-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  angle-double-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f102
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="angle-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  angle-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f107
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="angle-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  angle-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f104
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="angle-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  angle-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f105
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="angle-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  angle-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f106
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="angry"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  angry
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f556
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ankh"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ankh
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f644
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="apple-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  apple-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5d1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="archive"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  archive
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f187
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="archway"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  archway
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f557
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-alt-circle-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-alt-circle-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f358
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-alt-circle-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-alt-circle-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f359
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-alt-circle-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-alt-circle-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f35a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-alt-circle-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-alt-circle-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f35b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-circle-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-circle-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0ab
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-circle-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-circle-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0a8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-circle-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-circle-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0a9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-circle-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-circle-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0aa
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f063
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f060
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f061
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrow-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrow-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f062
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrows-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrows-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0b2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrows-alt-h"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrows-alt-h
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f337
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="arrows-alt-v"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  arrows-alt-v
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f338
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="assistive-listening-systems"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  assistive-listening-systems
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2a2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="asterisk"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  asterisk
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f069
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="at"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  at
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1fa
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="atlas"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  atlas
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f558
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="atom"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  atom
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5d2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="audio-description"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  audio-description
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f29e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="award"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  award
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f559
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="baby"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  baby
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f77c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="baby-carriage"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  baby-carriage
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f77d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="backspace"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  backspace
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f55a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="backward"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  backward
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f04a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bacon"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bacon
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7e5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bacteria"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bacteria
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e059
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bacterium"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bacterium
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e05a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bahai"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bahai
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f666
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="balance-scale"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  balance-scale
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f24e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="balance-scale-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  balance-scale-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f515
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="balance-scale-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  balance-scale-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f516
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ban"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ban
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f05e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="band-aid"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  band-aid
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f462
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="barcode"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  barcode
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f02a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bars"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bars
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0c9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="baseball-ball"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  baseball-ball
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f433
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="basketball-ball"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  basketball-ball
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f434
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bath"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bath
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2cd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="battery-empty"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  battery-empty
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f244
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="battery-full"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  battery-full
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f240
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="battery-half"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  battery-half
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f242
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="battery-quarter"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  battery-quarter
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f243
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="battery-three-quarters"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  battery-three-quarters
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f241
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bed"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bed
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f236
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="beer"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  beer
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0fc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bell"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bell
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0f3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bell-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bell-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1f6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bezier-curve"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bezier-curve
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f55b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bible"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bible
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f647
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bicycle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bicycle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f206
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="biking"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  biking
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f84a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="binoculars"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  binoculars
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1e5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="biohazard"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  biohazard
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f780
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="birthday-cake"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  birthday-cake
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1fd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="blender"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  blender
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f517
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="blender-phone"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  blender-phone
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6b6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="blind"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  blind
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f29d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="blog"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  blog
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f781
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bold"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bold
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f032
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bolt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bolt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0e7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bomb"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bomb
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1e2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bone"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bone
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5d7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bong"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bong
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f55c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="book"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  book
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f02d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="book-dead"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  book-dead
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6b7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="book-medical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  book-medical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7e6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="book-open"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  book-open
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f518
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="book-reader"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  book-reader
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5da
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bookmark"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bookmark
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f02e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="border-all"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  border-all
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f84c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="border-none"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  border-none
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f850
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="border-style"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  border-style
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f853
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bowling-ball"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bowling-ball
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f436
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="box"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  box
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f466
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="box-open"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  box-open
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f49e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="box-tissue"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  box-tissue
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e05b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="boxes"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  boxes
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f468
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="braille"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  braille
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2a1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="brain"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  brain
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5dc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bread-slice"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bread-slice
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7ec
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="briefcase"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  briefcase
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0b1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="briefcase-medical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  briefcase-medical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f469
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="broadcast-tower"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  broadcast-tower
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f519
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="broom"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  broom
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f51a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="brush"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  brush
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f55d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bug"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bug
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f188
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="building"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  building
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1ad
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bullhorn"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bullhorn
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0a1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bullseye"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bullseye
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f140
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="burn"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  burn
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f46a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f207
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="bus-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  bus-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f55e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="business-time"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  business-time
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f64a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="calculator"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  calculator
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1ec
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="calendar"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  calendar
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f133
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="calendar-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  calendar-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f073
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="calendar-check"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  calendar-check
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f274
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="calendar-day"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  calendar-day
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f783
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="calendar-minus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  calendar-minus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f272
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="calendar-plus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  calendar-plus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f271
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="calendar-times"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  calendar-times
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f273
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="calendar-week"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  calendar-week
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f784
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="camera"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  camera
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f030
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="camera-retro"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  camera-retro
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f083
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="campground"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  campground
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6bb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="candy-cane"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  candy-cane
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f786
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cannabis"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cannabis
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f55f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="capsules"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  capsules
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f46b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="car"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  car
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1b9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="car-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  car-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5de
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="car-battery"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  car-battery
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5df
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="car-crash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  car-crash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5e1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="car-side"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  car-side
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5e4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="caravan"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  caravan
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f8ff
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="caret-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  caret-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0d7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="caret-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  caret-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0d9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="caret-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  caret-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0da
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="caret-square-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  caret-square-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f150
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="caret-square-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  caret-square-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f191
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="caret-square-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  caret-square-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f152
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="caret-square-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  caret-square-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f151
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="caret-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  caret-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0d8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="carrot"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  carrot
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f787
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cart-arrow-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cart-arrow-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f218
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cart-plus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cart-plus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f217
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cash-register"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cash-register
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f788
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cat"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cat
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6be
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="certificate"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  certificate
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0a3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chair"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chair
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6c0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chalkboard"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chalkboard
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f51b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chalkboard-teacher"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chalkboard-teacher
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f51c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="charging-station"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  charging-station
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5e7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chart-area"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chart-area
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1fe
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chart-bar"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chart-bar
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f080
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chart-line"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chart-line
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f201
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chart-pie"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chart-pie
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f200
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="check"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  check
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f00c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="check-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  check-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f058
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="check-double"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  check-double
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f560
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="check-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  check-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f14a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cheese"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cheese
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7ef
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chess"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chess
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f439
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chess-bishop"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chess-bishop
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f43a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chess-board"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chess-board
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f43c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chess-king"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chess-king
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f43f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chess-knight"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chess-knight
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f441
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chess-pawn"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chess-pawn
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f443
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chess-queen"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chess-queen
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f445
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chess-rook"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chess-rook
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f447
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chevron-circle-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chevron-circle-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f13a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chevron-circle-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chevron-circle-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f137
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chevron-circle-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chevron-circle-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f138
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chevron-circle-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chevron-circle-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f139
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chevron-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chevron-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f078
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chevron-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chevron-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f053
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chevron-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chevron-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f054
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="chevron-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  chevron-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f077
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="child"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  child
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1ae
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="church"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  church
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f51d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f111
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="circle-notch"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  circle-notch
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1ce
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="city"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  city
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f64f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="clinic-medical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  clinic-medical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7f2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="clipboard"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  clipboard
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f328
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="clipboard-check"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  clipboard-check
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f46c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="clipboard-list"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  clipboard-list
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f46d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="clock"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  clock
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f017
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="clone"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  clone
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f24d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="closed-captioning"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  closed-captioning
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f20a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cloud"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cloud
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0c2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cloud-download-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cloud-download-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f381
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cloud-meatball"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cloud-meatball
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f73b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cloud-moon"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cloud-moon
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6c3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cloud-moon-rain"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cloud-moon-rain
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f73c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cloud-rain"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cloud-rain
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f73d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cloud-showers-heavy"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cloud-showers-heavy
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f740
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cloud-sun"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cloud-sun
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6c4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cloud-sun-rain"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cloud-sun-rain
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f743
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cloud-upload-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cloud-upload-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f382
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cocktail"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cocktail
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f561
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="code"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  code
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f121
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="code-branch"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  code-branch
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f126
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="coffee"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  coffee
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0f4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cog"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cog
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f013
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cogs"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cogs
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f085
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="coins"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  coins
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f51e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="columns"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  columns
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0db
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="comment"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  comment
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f075
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="comment-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  comment-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f27a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="comment-dollar"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  comment-dollar
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f651
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="comment-dots"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  comment-dots
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4ad
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="comment-medical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  comment-medical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7f5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="comment-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  comment-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4b3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="comments"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  comments
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f086
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="comments-dollar"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  comments-dollar
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f653
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="compact-disc"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  compact-disc
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f51f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="compass"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  compass
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f14e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="compress"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  compress
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f066
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="compress-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  compress-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f422
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="compress-arrows-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  compress-arrows-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f78c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="concierge-bell"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  concierge-bell
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f562
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cookie"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cookie
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f563
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cookie-bite"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cookie-bite
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f564
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="copy"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  copy
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0c5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="copyright"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  copyright
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1f9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="couch"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  couch
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4b8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="credit-card"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  credit-card
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f09d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="crop"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  crop
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f125
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="crop-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  crop-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f565
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cross"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cross
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f654
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="crosshairs"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  crosshairs
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f05b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="crow"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  crow
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f520
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="crown"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  crown
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f521
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="crutch"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  crutch
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7f7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cube"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cube
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1b2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cubes"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cubes
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1b3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="cut"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  cut
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0c4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="database"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  database
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1c0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="deaf"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  deaf
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2a4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="democrat"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  democrat
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f747
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="desktop"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  desktop
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f108
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dharmachakra"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dharmachakra
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f655
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="diagnoses"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  diagnoses
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f470
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dice"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dice
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f522
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dice-d20"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dice-d20
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6cf
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dice-d6"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dice-d6
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6d1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dice-five"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dice-five
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f523
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dice-four"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dice-four
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f524
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dice-one"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dice-one
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f525
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dice-six"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dice-six
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f526
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dice-three"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dice-three
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f527
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dice-two"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dice-two
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f528
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="digital-tachograph"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  digital-tachograph
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f566
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="directions"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  directions
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5eb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="disease"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  disease
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7fa
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="divide"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  divide
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f529
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dizzy"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dizzy
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f567
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dna"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dna
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f471
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dog"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dog
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6d3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dollar-sign"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dollar-sign
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f155
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dolly"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dolly
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f472
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dolly-flatbed"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dolly-flatbed
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f474
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="donate"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  donate
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4b9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="door-closed"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  door-closed
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f52a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="door-open"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  door-open
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f52b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dot-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dot-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f192
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dove"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dove
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4ba
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="download"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  download
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f019
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="drafting-compass"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  drafting-compass
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f568
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dragon"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dragon
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6d5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="draw-polygon"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  draw-polygon
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5ee
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="drum"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  drum
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f569
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="drum-steelpan"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  drum-steelpan
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f56a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="drumstick-bite"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  drumstick-bite
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6d7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dumbbell"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dumbbell
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f44b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dumpster"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dumpster
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f793
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dumpster-fire"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dumpster-fire
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f794
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="dungeon"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  dungeon
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6d9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="edit"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  edit
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f044
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="egg"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  egg
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7fb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="eject"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  eject
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f052
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ellipsis-h"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ellipsis-h
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f141
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ellipsis-v"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ellipsis-v
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f142
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="envelope"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  envelope
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0e0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="envelope-open"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  envelope-open
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2b6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="envelope-open-text"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  envelope-open-text
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f658
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="envelope-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  envelope-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f199
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="equals"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  equals
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f52c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="eraser"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  eraser
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f12d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ethernet"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ethernet
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f796
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="euro-sign"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  euro-sign
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f153
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="exchange-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  exchange-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f362
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="exclamation"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  exclamation
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f12a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="exclamation-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  exclamation-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f06a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="exclamation-triangle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  exclamation-triangle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f071
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="expand"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  expand
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f065
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="expand-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  expand-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f424
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="expand-arrows-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  expand-arrows-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f31e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="external-link-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  external-link-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f35d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="external-link-square-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  external-link-square-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f360
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="eye"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  eye
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f06e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="eye-dropper"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  eye-dropper
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1fb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="eye-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  eye-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f070
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fan"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fan
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f863
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fast-backward"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fast-backward
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f049
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fast-forward"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fast-forward
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f050
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="faucet"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  faucet
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e005
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fax"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fax
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1ac
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="feather"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  feather
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f52d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="feather-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  feather-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f56b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="female"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  female
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f182
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fighter-jet"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fighter-jet
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0fb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f15b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f15c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-archive"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-archive
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1c6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-audio"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-audio
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1c7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-code"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-code
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1c9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-contract"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-contract
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f56c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-csv"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-csv
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6dd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-download"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-download
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f56d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-excel"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-excel
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1c3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-export"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-export
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f56e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-image"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-image
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1c5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-import"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-import
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f56f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-invoice"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-invoice
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f570
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-invoice-dollar"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-invoice-dollar
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f571
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-medical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-medical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f477
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-medical-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-medical-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f478
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-pdf"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-pdf
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1c1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-powerpoint"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-powerpoint
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1c4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-prescription"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-prescription
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f572
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-signature"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-signature
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f573
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-upload"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-upload
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f574
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-video"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-video
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1c8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="file-word"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  file-word
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1c2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fill"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fill
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f575
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fill-drip"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fill-drip
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f576
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="film"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  film
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f008
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="filter"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  filter
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0b0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fingerprint"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fingerprint
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f577
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fire"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fire
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f06d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fire-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fire-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7e4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fire-extinguisher"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fire-extinguisher
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f134
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="first-aid"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  first-aid
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f479
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fish"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fish
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f578
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="fist-raised"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  fist-raised
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6de
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="flag"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  flag
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f024
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="flag-checkered"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  flag-checkered
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f11e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="flag-usa"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  flag-usa
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f74d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="flask"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  flask
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0c3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="flushed"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  flushed
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f579
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="folder"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  folder
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f07b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="folder-minus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  folder-minus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f65d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="folder-open"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  folder-open
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f07c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="folder-plus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  folder-plus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f65e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="font"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  font
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f031
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="football-ball"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  football-ball
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f44e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="forward"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  forward
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f04e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="frog"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  frog
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f52e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="frown"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  frown
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f119
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="frown-open"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  frown-open
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f57a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="funnel-dollar"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  funnel-dollar
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f662
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="futbol"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  futbol
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1e3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="gamepad"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  gamepad
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f11b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="gas-pump"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  gas-pump
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f52f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="gavel"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  gavel
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0e3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="gem"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  gem
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3a5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="genderless"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  genderless
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f22d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ghost"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ghost
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6e2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="gift"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  gift
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f06b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="gifts"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  gifts
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f79c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="glass-cheers"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  glass-cheers
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f79f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="glass-martini"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  glass-martini
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f000
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="glass-martini-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  glass-martini-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f57b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="glass-whiskey"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  glass-whiskey
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7a0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="glasses"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  glasses
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f530
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="globe"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  globe
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0ac
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="globe-africa"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  globe-africa
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f57c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="globe-americas"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  globe-americas
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f57d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="globe-asia"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  globe-asia
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f57e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="globe-europe"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  globe-europe
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7a2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="golf-ball"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  golf-ball
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f450
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="gopuram"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  gopuram
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f664
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="graduation-cap"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  graduation-cap
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f19d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="greater-than"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  greater-than
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f531
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="greater-than-equal"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  greater-than-equal
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f532
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grimace"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grimace
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f57f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f580
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f581
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-beam"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-beam
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f582
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-beam-sweat"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-beam-sweat
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f583
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-hearts"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-hearts
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f584
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-squint"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-squint
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f585
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-squint-tears"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-squint-tears
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f586
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-stars"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-stars
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f587
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-tears"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-tears
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f588
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-tongue"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-tongue
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f589
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-tongue-squint"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-tongue-squint
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f58a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-tongue-wink"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-tongue-wink
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f58b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grin-wink"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grin-wink
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f58c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grip-horizontal"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grip-horizontal
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f58d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grip-lines"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grip-lines
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7a4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grip-lines-vertical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grip-lines-vertical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7a5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="grip-vertical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  grip-vertical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f58e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="guitar"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  guitar
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7a6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="h-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  h-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0fd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hamburger"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hamburger
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f805
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hammer"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hammer
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6e3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hamsa"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hamsa
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f665
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-holding"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-holding
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4bd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-holding-heart"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-holding-heart
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4be
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-holding-medical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-holding-medical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e05c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-holding-usd"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-holding-usd
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4c0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-holding-water"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-holding-water
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4c1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-lizard"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-lizard
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f258
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-middle-finger"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-middle-finger
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f806
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-paper"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-paper
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f256
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-peace"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-peace
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f25b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-point-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-point-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0a7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-point-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-point-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0a5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-point-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-point-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0a4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-point-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-point-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0a6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-pointer"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-pointer
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f25a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-rock"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-rock
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f255
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-scissors"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-scissors
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f257
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-sparkles"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-sparkles
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e05d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hand-spock"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hand-spock
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f259
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hands"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hands
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4c2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hands-helping"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hands-helping
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4c4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hands-wash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hands-wash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e05e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="handshake"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  handshake
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2b5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="handshake-alt-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  handshake-alt-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e05f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="handshake-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  handshake-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e060
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hanukiah"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hanukiah
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6e6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hard-hat"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hard-hat
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f807
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hashtag"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hashtag
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f292
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hat-cowboy"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hat-cowboy
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f8c0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hat-cowboy-side"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hat-cowboy-side
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f8c1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hat-wizard"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hat-wizard
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6e8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hdd"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hdd
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0a0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="head-side-cough"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  head-side-cough
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e061
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="head-side-cough-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  head-side-cough-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e062
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="head-side-mask"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  head-side-mask
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e063
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="head-side-virus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  head-side-virus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e064
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="heading"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  heading
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1dc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="headphones"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  headphones
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f025
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="headphones-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  headphones-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f58f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="headset"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  headset
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f590
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="heart"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  heart
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f004
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="heart-broken"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  heart-broken
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7a9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="heartbeat"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  heartbeat
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f21e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="helicopter"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  helicopter
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f533
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="highlighter"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  highlighter
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f591
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hiking"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hiking
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6ec
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hippo"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hippo
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6ed
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="history"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  history
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1da
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hockey-puck"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hockey-puck
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f453
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="holly-berry"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  holly-berry
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7aa
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="home"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  home
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f015
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="horse"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  horse
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6f0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="horse-head"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  horse-head
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7ab
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hospital"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hospital
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0f8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hospital-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hospital-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f47d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hospital-symbol"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hospital-symbol
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f47e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hospital-user"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hospital-user
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f80d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hot-tub"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hot-tub
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f593
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hotdog"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hotdog
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f80f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hotel"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hotel
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f594
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hourglass"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hourglass
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f254
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hourglass-end"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hourglass-end
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f253
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hourglass-half"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hourglass-half
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f252
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hourglass-start"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hourglass-start
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f251
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="house-damage"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  house-damage
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6f1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="house-user"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  house-user
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e065
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="hryvnia"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  hryvnia
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6f2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="i-cursor"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  i-cursor
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f246
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ice-cream"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ice-cream
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f810
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="icicles"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  icicles
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7ad
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="icons"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  icons
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f86d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="id-badge"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  id-badge
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2c1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="id-card"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  id-card
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2c2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="id-card-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  id-card-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f47f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="igloo"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  igloo
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7ae
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="image"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  image
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f03e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="images"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  images
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f302
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="inbox"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  inbox
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f01c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="indent"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  indent
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f03c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="industry"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  industry
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f275
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="infinity"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  infinity
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f534
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="info"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  info
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f129
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="info-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  info-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f05a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="italic"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  italic
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f033
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="jedi"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  jedi
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f669
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="joint"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  joint
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f595
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="journal-whills"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  journal-whills
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f66a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="kaaba"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  kaaba
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f66b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="key"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  key
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f084
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="keyboard"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  keyboard
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f11c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="khanda"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  khanda
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f66d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="kiss"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  kiss
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f596
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="kiss-beam"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  kiss-beam
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f597
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="kiss-wink-heart"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  kiss-wink-heart
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f598
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="kiwi-bird"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  kiwi-bird
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f535
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="landmark"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  landmark
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f66f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="language"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  language
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1ab
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="laptop"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  laptop
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f109
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="laptop-code"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  laptop-code
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5fc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="laptop-house"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  laptop-house
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e066
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="laptop-medical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  laptop-medical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f812
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="laugh"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  laugh
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f599
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="laugh-beam"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  laugh-beam
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f59a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="laugh-squint"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  laugh-squint
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f59b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="laugh-wink"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  laugh-wink
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f59c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="layer-group"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  layer-group
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5fd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="leaf"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  leaf
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f06c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="lemon"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  lemon
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f094
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="less-than"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  less-than
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f536
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="less-than-equal"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  less-than-equal
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f537
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="level-down-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  level-down-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3be
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="level-up-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  level-up-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3bf
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="life-ring"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  life-ring
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1cd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="lightbulb"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  lightbulb
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0eb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="link"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  link
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0c1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="lira-sign"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  lira-sign
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f195
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="list"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  list
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f03a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="list-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  list-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f022
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="list-ol"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  list-ol
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0cb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="list-ul"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  list-ul
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0ca
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="location-arrow"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  location-arrow
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f124
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="lock"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  lock
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f023
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="lock-open"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  lock-open
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3c1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="long-arrow-alt-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  long-arrow-alt-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f309
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="long-arrow-alt-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  long-arrow-alt-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f30a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="long-arrow-alt-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  long-arrow-alt-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f30b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="long-arrow-alt-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  long-arrow-alt-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f30c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="low-vision"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  low-vision
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2a8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="luggage-cart"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  luggage-cart
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f59d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="lungs"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  lungs
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f604
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="lungs-virus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  lungs-virus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e067
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="magic"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  magic
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0d0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="magnet"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  magnet
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f076
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mail-bulk"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mail-bulk
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f674
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="male"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  male
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f183
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="map"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  map
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f279
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="map-marked"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  map-marked
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f59f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="map-marked-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  map-marked-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5a0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="map-marker"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  map-marker
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f041
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="map-marker-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  map-marker-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3c5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="map-pin"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  map-pin
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f276
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="map-signs"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  map-signs
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f277
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="marker"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  marker
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5a1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mars"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mars
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f222
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mars-double"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mars-double
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f227
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mars-stroke"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mars-stroke
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f229
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mars-stroke-h"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mars-stroke-h
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f22b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mars-stroke-v"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mars-stroke-v
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f22a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mask"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mask
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6fa
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="medal"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  medal
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5a2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="medkit"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  medkit
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0fa
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="meh"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  meh
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f11a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="meh-blank"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  meh-blank
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5a4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="meh-rolling-eyes"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  meh-rolling-eyes
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5a5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="memory"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  memory
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f538
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="menorah"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  menorah
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f676
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mercury"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mercury
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f223
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="meteor"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  meteor
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f753
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="microchip"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  microchip
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2db
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="microphone"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  microphone
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f130
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="microphone-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  microphone-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3c9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="microphone-alt-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  microphone-alt-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f539
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="microphone-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  microphone-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f131
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="microscope"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  microscope
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f610
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="minus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  minus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f068
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="minus-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  minus-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f056
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="minus-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  minus-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f146
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mitten"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mitten
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7b5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mobile"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mobile
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f10b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mobile-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mobile-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3cd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="money-bill"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  money-bill
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0d6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="money-bill-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  money-bill-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3d1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="money-bill-wave"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  money-bill-wave
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f53a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="money-bill-wave-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  money-bill-wave-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f53b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="money-check"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  money-check
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f53c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="money-check-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  money-check-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f53d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="monument"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  monument
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5a6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="moon"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  moon
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f186
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mortar-pestle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mortar-pestle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5a7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mosque"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mosque
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f678
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="motorcycle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  motorcycle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f21c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mountain"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mountain
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6fc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mouse"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mouse
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f8cc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mouse-pointer"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mouse-pointer
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f245
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="mug-hot"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  mug-hot
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7b6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="music"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  music
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f001
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="network-wired"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  network-wired
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6ff
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="neuter"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  neuter
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f22c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="newspaper"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  newspaper
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1ea
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="not-equal"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  not-equal
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f53e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="notes-medical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  notes-medical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f481
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="object-group"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  object-group
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f247
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="object-ungroup"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  object-ungroup
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f248
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="oil-can"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  oil-can
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f613
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="om"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  om
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f679
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="otter"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  otter
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f700
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="outdent"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  outdent
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f03b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pager"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pager
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f815
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="paint-brush"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  paint-brush
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1fc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="paint-roller"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  paint-roller
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5aa
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="palette"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  palette
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f53f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pallet"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pallet
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f482
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="paper-plane"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  paper-plane
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1d8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="paperclip"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  paperclip
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0c6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="parachute-box"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  parachute-box
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4cd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="paragraph"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  paragraph
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1dd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="parking"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  parking
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f540
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="passport"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  passport
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5ab
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pastafarianism"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pastafarianism
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f67b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="paste"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  paste
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0ea
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pause"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pause
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f04c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pause-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pause-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f28b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="paw"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  paw
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1b0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="peace"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  peace
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f67c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pen"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pen
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f304
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pen-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pen-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f305
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pen-fancy"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pen-fancy
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5ac
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pen-nib"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pen-nib
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5ad
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pen-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pen-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f14b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pencil-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pencil-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f303
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pencil-ruler"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pencil-ruler
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5ae
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="people-arrows"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  people-arrows
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e068
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="people-carry"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  people-carry
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4ce
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pepper-hot"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pepper-hot
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f816
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="percent"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  percent
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f295
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="percentage"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  percentage
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f541
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="person-booth"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  person-booth
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f756
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="phone"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  phone
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f095
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="phone-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  phone-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f879
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="phone-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  phone-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3dd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="phone-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  phone-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f098
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="phone-square-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  phone-square-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f87b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="phone-volume"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  phone-volume
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2a0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="photo-video"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  photo-video
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f87c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="piggy-bank"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  piggy-bank
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4d3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pills"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pills
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f484
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pizza-slice"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pizza-slice
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f818
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="place-of-worship"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  place-of-worship
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f67f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="plane"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  plane
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f072
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="plane-arrival"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  plane-arrival
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5af
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="plane-departure"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  plane-departure
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5b0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="plane-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  plane-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e069
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="play"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  play
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f04b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="play-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  play-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f144
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="plug"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  plug
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1e6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="plus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  plus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f067
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="plus-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  plus-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f055
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="plus-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  plus-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0fe
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="podcast"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  podcast
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2ce
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="poll"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  poll
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f681
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="poll-h"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  poll-h
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f682
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="poo"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  poo
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2fe
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="poo-storm"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  poo-storm
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f75a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="poop"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  poop
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f619
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="portrait"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  portrait
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3e0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pound-sign"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pound-sign
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f154
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="power-off"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  power-off
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f011
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pray"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pray
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f683
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="praying-hands"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  praying-hands
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f684
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="prescription"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  prescription
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5b1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="prescription-bottle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  prescription-bottle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f485
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="prescription-bottle-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  prescription-bottle-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f486
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="print"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  print
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f02f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="procedures"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  procedures
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f487
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="project-diagram"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  project-diagram
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f542
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pump-medical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pump-medical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e06a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="pump-soap"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  pump-soap
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e06b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="puzzle-piece"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  puzzle-piece
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f12e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="qrcode"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  qrcode
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f029
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="question"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  question
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f128
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="question-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  question-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f059
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="quidditch"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  quidditch
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f458
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="quote-left"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  quote-left
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f10d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="quote-right"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  quote-right
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f10e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="quran"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  quran
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f687
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="radiation"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  radiation
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7b9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="radiation-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  radiation-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7ba
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="rainbow"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  rainbow
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f75b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="random"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  random
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f074
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="receipt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  receipt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f543
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="record-vinyl"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  record-vinyl
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f8d9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="recycle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  recycle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1b8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="redo"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  redo
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f01e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="redo-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  redo-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2f9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="registered"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  registered
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f25d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="remove-format"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  remove-format
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f87d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="reply"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  reply
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3e5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="reply-all"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  reply-all
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f122
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="republican"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  republican
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f75e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="restroom"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  restroom
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7bd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="retweet"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  retweet
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f079
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ribbon"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ribbon
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4d6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ring"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ring
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f70b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="road"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  road
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f018
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="robot"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  robot
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f544
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="rocket"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  rocket
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f135
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="route"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  route
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4d7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="rss"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  rss
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f09e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="rss-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  rss-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f143
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ruble-sign"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ruble-sign
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f158
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ruler"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ruler
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f545
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ruler-combined"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ruler-combined
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f546
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ruler-horizontal"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ruler-horizontal
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f547
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ruler-vertical"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ruler-vertical
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f548
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="running"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  running
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f70c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="rupee-sign"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  rupee-sign
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f156
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sad-cry"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sad-cry
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5b3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sad-tear"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sad-tear
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5b4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="satellite"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  satellite
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7bf
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="satellite-dish"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  satellite-dish
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7c0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="save"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  save
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0c7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="school"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  school
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f549
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="screwdriver"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  screwdriver
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f54a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="scroll"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  scroll
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f70e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sd-card"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sd-card
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7c2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="search"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  search
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f002
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="search-dollar"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  search-dollar
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f688
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="search-location"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  search-location
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f689
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="search-minus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  search-minus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f010
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="search-plus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  search-plus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f00e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="seedling"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  seedling
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4d8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="server"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  server
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f233
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="shapes"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  shapes
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f61f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="share"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  share
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f064
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="share-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  share-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1e0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="share-alt-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  share-alt-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1e1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="share-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  share-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f14d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="shekel-sign"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  shekel-sign
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f20b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="shield-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  shield-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3ed
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="shield-virus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  shield-virus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e06c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ship"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ship
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f21a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="shipping-fast"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  shipping-fast
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f48b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="shoe-prints"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  shoe-prints
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f54b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="shopping-bag"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  shopping-bag
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f290
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="shopping-basket"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  shopping-basket
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f291
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="shopping-cart"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  shopping-cart
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f07a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="shower"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  shower
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2cc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="shuttle-van"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  shuttle-van
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5b6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sign"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sign
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4d9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sign-in-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sign-in-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2f6
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sign-language"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sign-language
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2a7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sign-out-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sign-out-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2f5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="signal"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  signal
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f012
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="signature"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  signature
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5b7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sim-card"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sim-card
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7c4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sink"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sink
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e06d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sitemap"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sitemap
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0e8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="skating"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  skating
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7c5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="skiing"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  skiing
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7c9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="skiing-nordic"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  skiing-nordic
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7ca
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="skull"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  skull
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f54c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="skull-crossbones"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  skull-crossbones
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f714
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f715
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sleigh"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sleigh
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7cc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sliders-h"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sliders-h
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1de
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="smile"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  smile
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f118
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="smile-beam"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  smile-beam
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5b8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="smile-wink"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  smile-wink
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4da
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="smog"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  smog
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f75f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="smoking"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  smoking
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f48d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="smoking-ban"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  smoking-ban
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f54d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sms"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sms
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7cd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="snowboarding"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  snowboarding
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7ce
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="snowflake"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  snowflake
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2dc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="snowman"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  snowman
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7d0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="snowplow"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  snowplow
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7d2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="soap"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  soap
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e06e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="socks"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  socks
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f696
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="solar-panel"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  solar-panel
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5ba
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0dc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-alpha-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-alpha-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f15d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-alpha-down-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-alpha-down-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f881
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-alpha-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-alpha-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f15e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-alpha-up-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-alpha-up-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f882
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-amount-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-amount-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f160
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-amount-down-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-amount-down-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f884
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-amount-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-amount-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f161
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-amount-up-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-amount-up-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f885
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0dd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-numeric-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-numeric-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f162
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-numeric-down-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-numeric-down-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f886
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-numeric-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-numeric-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f163
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-numeric-up-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-numeric-up-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f887
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sort-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sort-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0de
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="spa"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  spa
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5bb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="space-shuttle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  space-shuttle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f197
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="spell-check"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  spell-check
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f891
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="spider"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  spider
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f717
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="spinner"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  spinner
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f110
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="splotch"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  splotch
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5bc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="spray-can"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  spray-can
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5bd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0c8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="square-full"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  square-full
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f45c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="square-root-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  square-root-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f698
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="stamp"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  stamp
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5bf
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="star"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  star
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f005
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="star-and-crescent"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  star-and-crescent
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f699
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="star-half"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  star-half
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f089
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="star-half-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  star-half-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5c0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="star-of-david"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  star-of-david
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f69a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="star-of-life"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  star-of-life
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f621
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="step-backward"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  step-backward
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f048
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="step-forward"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  step-forward
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f051
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="stethoscope"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  stethoscope
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0f1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sticky-note"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sticky-note
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f249
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="stop"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  stop
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f04d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="stop-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  stop-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f28d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="stopwatch"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  stopwatch
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2f2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="stopwatch-20"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  stopwatch-20
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e06f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="store"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  store
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f54e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="store-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  store-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f54f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="store-alt-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  store-alt-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e070
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="store-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  store-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e071
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="stream"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  stream
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f550
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="street-view"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  street-view
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f21d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="strikethrough"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  strikethrough
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0cc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="stroopwafel"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  stroopwafel
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f551
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="subscript"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  subscript
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f12c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="subway"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  subway
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f239
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="suitcase"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  suitcase
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0f2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="suitcase-rolling"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  suitcase-rolling
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5c1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sun"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sun
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f185
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="superscript"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  superscript
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f12b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="surprise"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  surprise
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5c2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="swatchbook"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  swatchbook
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5c3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="swimmer"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  swimmer
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5c4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="swimming-pool"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  swimming-pool
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5c5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="synagogue"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  synagogue
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f69b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sync"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sync
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f021
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="sync-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  sync-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2f1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="syringe"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  syringe
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f48e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="table"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  table
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0ce
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="table-tennis"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  table-tennis
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f45d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tablet"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tablet
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f10a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tablet-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tablet-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3fa
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tablets"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tablets
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f490
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tachometer-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tachometer-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3fd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tag"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tag
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f02b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tags"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tags
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f02c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tape"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tape
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4db
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tasks"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tasks
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0ae
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="taxi"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  taxi
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1ba
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="teeth"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  teeth
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f62e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="teeth-open"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  teeth-open
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f62f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="temperature-high"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  temperature-high
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f769
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="temperature-low"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  temperature-low
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f76b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tenge"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tenge
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7d7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="terminal"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  terminal
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f120
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="text-height"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  text-height
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f034
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="text-width"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  text-width
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f035
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="th"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  th
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f00a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="th-large"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  th-large
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f009
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="th-list"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  th-list
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f00b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="theater-masks"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  theater-masks
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f630
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="thermometer"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  thermometer
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f491
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="thermometer-empty"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  thermometer-empty
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2cb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="thermometer-full"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  thermometer-full
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2c7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="thermometer-half"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  thermometer-half
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2c9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="thermometer-quarter"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  thermometer-quarter
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2ca
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="thermometer-three-quarters"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  thermometer-three-quarters
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2c8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="thumbs-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  thumbs-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f165
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="thumbs-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  thumbs-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f164
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="thumbtack"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  thumbtack
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f08d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="ticket-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  ticket-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f3ff
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="times"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  times
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f00d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="times-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  times-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f057
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tint"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tint
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f043
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tint-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tint-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5c7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tired"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tired
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5c8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="toggle-off"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  toggle-off
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f204
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="toggle-on"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  toggle-on
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f205
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="toilet"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  toilet
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7d8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="toilet-paper"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  toilet-paper
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f71e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="toilet-paper-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  toilet-paper-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e072
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="toolbox"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  toolbox
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f552
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tools"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tools
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7d9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tooth"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tooth
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5c9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="torah"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  torah
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6a0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="torii-gate"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  torii-gate
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6a1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tractor"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tractor
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f722
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="trademark"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  trademark
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f25c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="traffic-light"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  traffic-light
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f637
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="trailer"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  trailer
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e041
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="train"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  train
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f238
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tram"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tram
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f7da
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="transgender"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  transgender
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f224
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="transgender-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  transgender-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f225
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="trash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  trash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1f8
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="trash-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  trash-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2ed
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="trash-restore"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  trash-restore
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f829
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="trash-restore-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  trash-restore-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f82a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tree"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tree
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1bb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="trophy"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  trophy
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f091
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="truck"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  truck
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0d1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="truck-loading"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  truck-loading
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4de
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="truck-monster"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  truck-monster
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f63b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="truck-moving"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  truck-moving
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4df
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="truck-pickup"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  truck-pickup
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f63c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tshirt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tshirt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f553
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tty"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tty
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1e4
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="tv"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  tv
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f26c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="umbrella"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  umbrella
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0e9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="umbrella-beach"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  umbrella-beach
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5ca
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="underline"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  underline
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0cd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="undo"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  undo
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0e2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="undo-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  undo-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2ea
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="universal-access"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  universal-access
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f29a
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="university"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  university
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f19c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="unlink"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  unlink
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f127
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="unlock"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  unlock
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f09c
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="unlock-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  unlock-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f13e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="upload"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  upload
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f093
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f007
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f406
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-alt-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-alt-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4fa
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-astronaut"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-astronaut
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4fb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-check"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-check
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4fc
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-circle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-circle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2bd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-clock"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-clock
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4fd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-cog"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-cog
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4fe
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-edit"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-edit
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4ff
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-friends"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-friends
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f500
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-graduate"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-graduate
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f501
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-injured"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-injured
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f728
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-lock"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-lock
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f502
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-md"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-md
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0f0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-minus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-minus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f503
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-ninja"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-ninja
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f504
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-nurse"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-nurse
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f82f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-plus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-plus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f234
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-secret"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-secret
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f21b
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-shield"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-shield
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f505
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f506
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-tag"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-tag
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f507
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-tie"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-tie
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f508
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="user-times"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  user-times
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f235
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="users"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  users
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0c0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="users-cog"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  users-cog
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f509
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="users-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  users-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e073
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="utensil-spoon"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  utensil-spoon
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2e5
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="utensils"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  utensils
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2e7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="vector-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  vector-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5cb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="venus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  venus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f221
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="venus-double"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  venus-double
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f226
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="venus-mars"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  venus-mars
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f228
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="vial"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  vial
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f492
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="vials"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  vials
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f493
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="video"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  video
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f03d
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="video-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  video-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4e2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="vihara"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  vihara
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6a7
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="virus"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  virus
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e074
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="virus-slash"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  virus-slash
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e075
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="viruses"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  viruses
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  e076
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="voicemail"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  voicemail
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f897
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="volleyball-ball"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  volleyball-ball
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f45f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="volume-down"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  volume-down
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f027
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="volume-mute"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  volume-mute
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6a9
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="volume-off"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  volume-off
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f026
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="volume-up"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  volume-up
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f028
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="vote-yea"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  vote-yea
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f772
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="vr-cardboard"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  vr-cardboard
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f729
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="walking"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  walking
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f554
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="wallet"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  wallet
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f555
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="warehouse"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  warehouse
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f494
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="water"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  water
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f773
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="wave-square"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  wave-square
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f83e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="weight"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  weight
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f496
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="weight-hanging"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  weight-hanging
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5cd
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="wheelchair"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  wheelchair
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f193
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="wifi"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  wifi
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f1eb
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="wind"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  wind
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f72e
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="window-close"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  window-close
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f410
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="window-maximize"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  window-maximize
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2d0
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="window-minimize"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  window-minimize
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2d1
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="window-restore"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  window-restore
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f2d2
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="wine-bottle"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  wine-bottle
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f72f
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="wine-glass"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  wine-glass
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f4e3
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="wine-glass-alt"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  wine-glass-alt
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f5ce
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="won-sign"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  won-sign
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f159
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="wrench"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  wrench
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f0ad
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="x-ray"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  x-ray
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f497
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="yen-sign"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  yen-sign
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f157
                </dd>
              </dl>
              {/**/}
            </article>
            <article
              id="yin-yang"
              className="icon col-6 col-md-3 col-lg-2 pr4 pb2 pt2 bb bw1 b--gray1 hover-black bw0-pr db fl-pr"
            >
              <dl className="dt w-100 ma0 pa0">
                <dt className="the-icon">
                  <span className="fa-fw select-all fas"></span>
                </dt>
                <dd className="mt-2 text-sm select-all word-wrap dtc v-top tl f2 icon-name">
                  yin-yang
                </dd>
                <dd className="text-xs text-muted gray5 dtc v-top tr f2 icon-unicode">
                  f6ad
                </dd>
              </dl>
              {/**/}
            </article>
          </div>
        </div>
      </div>
    </section>
  </div>
  {"{"}% endblock %{"}"}
  {"{"}% block styles %{"}"}
  <link
    rel="stylesheet"
    href="assets/extensions/@fortawesome/fontawesome-free/css/all.min.css"
  />
  <style
    dangerouslySetInnerHTML={{
      __html:
        "\n    .fontawesome-icons {\n        text-align: center;\n    }\n\n    article dl {\n        background-color: rgba(0, 0, 0, .02);\n        padding: 20px;\n    }\n\n    .fontawesome-icons .the-icon {\n        font-size: 24px;\n        line-height: 1.2;\n    }\n"
    }}
  />
  {"{"}% endblock %{"}"}
</>
